...
function upload(selector) {
  ...
  var open = element('btn', ['btn'], 'Open');
  var upLoad = element('btn', ['btn', 'primary'], 'Load');
...
(0, _upload.upload)('#file', {
  ...
  onUpload: function onUpload(files, blocks) {
    files.forEach(function (file, index) {
      var ref = storage.ref("image/".concat(file.name));
      var task = ref.put(file);
      task.on('state_changed', function (snapshot) {
        var persentage = (snapshot.bytesTransferred / snapshot.totalBytes * 100).toFixed(0) + '%';
